...
class CheckoutTest(unittest.TestCase):
    ...
    def test_clean(self):
        ...
        with Stub(os, 'chdir', Pass):
            ci = bootstrap.CheckoutInfo('', PULL, None)
            bootstrap.checkout(fake, REPO, ci, clean=True)
    ...
    def test_fetch_retries(self):
        ...
        with Stub(os, 'chdir', Pass):
            with Stub(time, 'sleep', Pass):
                ci = bootstrap.CheckoutInfo('', PULL, None)
                bootstrap.checkout(third_time_charm, REPO, ci)
    ...
    def test_pull_ref(self):
        ...
        with Stub(os, 'chdir', Pass):
            ci = bootstrap.CheckoutInfo('', PULL, None)
            bootstrap.checkout(fake, REPO, ci)
    ...
    def test_branch(self):
        ...
        fake = FakeSubprocess()
        with Stub(os, 'chdir', Pass):
            ci = bootstrap.CheckoutInfo(BRANCH, '', None)
            bootstrap.checkout(fake, REPO, ci)
    ...
    def test_branch_xor_pull(self):
        ...
        with Stub(os, 'chdir', Bomb):
            with self.assertRaises(ValueError):
                bootstrap.checkout(Bomb, REPO, bootstrap.CheckoutInfo('', '', None))
            with self.assertRaises(ValueError):
                ci = bootstrap.CheckoutInfo(BRANCH, PULL, None)
                bootstrap.checkout(Bomb, REPO, ci)
...
class ParseReposTest(unittest.TestCase):
    ...
    def test_deprecated_branch(self):
        ...
        self.assertEquals(
            {'foo': bootstrap.CheckoutInfo('bbb', '', None)},
    ...
    def test_deprecated_branch_commit(self):
        ...
        self.assertEquals(
            {'foo': bootstrap.CheckoutInfo('bbb:1234', '', None)},
    ...
    def test_deprecated_pull(self):
        ...
        self.assertEquals(
            {'foo': bootstrap.CheckoutInfo('', '123:abc,333:ddd', None)},
    ...
    def test_plain(self):
        ...
        self.assertEquals(
            {'foo': bootstrap.CheckoutInfo('master', '', None)},
            bootstrap.parse_repos(FakeArgs(repo=['foo'], branch='', pull='')))

    def test_branch(self):
        """--repo=foo=branch."""
        self.assertEquals(
            {'foo': bootstrap.CheckoutInfo('this', '', None)},
    ...
    def test_branch_commit(self):
        ...
        self.assertEquals(
            {'foo': bootstrap.CheckoutInfo('this:abcd', '', None)},
            bootstrap.parse_repos(
    ...
    def test_parse_repos(self):
        ...
        self.assertEquals(
            {'foo': bootstrap.CheckoutInfo('', '111,222', None)},
            bootstrap.parse_repos(FakeArgs(
    ...
    def test_pull_release_branch(self):
        ...
        self.assertEquals(
            {'foo': bootstrap.CheckoutInfo(
                '', 'release-3.14,&a-fancy%_branch+:abcd,222', None)},
            bootstrap.parse_repos(
    ...
    def test_pull_branch_commit(self):
        ...
        self.assertEquals(
            {'foo': bootstrap.CheckoutInfo(
                '', 'master:aaa,111:bbb,222:ccc', None)},
            bootstrap.parse_repos(FakeArgs(
...
class FinishTest(unittest.TestCase):
    ...
    def test_no_version(self):
        ...
        repos = repo({REPO: bootstrap.CheckoutInfo('master', '', None)})
...
class MetadataTest(unittest.TestCase):
    ...
    def test_always_set_metadata(self):
        repos = repo({
            REPO: bootstrap.CheckoutInfo('master', '', None)})
    ...
    def test_multi_repo(self):
        repos = repo({
            REPO: bootstrap.CheckoutInfo('foo', '', None), 
            'other-repo': bootstrap.CheckoutInfo('', '123,456', None)})
        meta = bootstrap.metadata(repos, 'missing-artifacts-dir', FakeCall())
        self.assertIn('repo', meta)
        self.assertEquals(REPO, meta['repo'])
...
class PRPathsTest(unittest.TestCase):
    def test_kubernetes_kubernetes(self):
        ...
        path = bootstrap.pr_paths(UPLOAD, repo({'kubernetes/kubernetes': 
            bootstrap.CheckoutInfo('', PULL, None)}), JOB, BUILD)
        self.assertTrue(any(
            str(PULL) == p for p in path.build_log.split('/')))

    def test_kubernetes(self):
        """Test the kubernetes/something prefix."""
        path = bootstrap.pr_paths(UPLOAD, repo({'kubernetes/prefix': 
            bootstrap.CheckoutInfo('', PULL, None)}), JOB, BUILD)
        self.assertTrue(any(
    ...
    def test_other(self):
        ...
        path = bootstrap.pr_paths(UPLOAD, repo({'github.com/random/repo': 
            bootstrap.CheckoutInfo('', PULL, None)}), JOB, BUILD)
        self.assertTrue(any(
...
class FakeArgs(object):
    ...
    json = False
    no_magic_env = False
    jobs_dir = os.path.join(os.path.dirname(__file__), 'test_jobs')
    repo_manifest = ''
...
class IntegrationTest(unittest.TestCase):
    ...
    def test_commit_in_meta(self):
        ...
        repos = repo({
            REPO: bootstrap.CheckoutInfo('master', '', None), 
            'other-repo': bootstrap.CheckoutInfo('other-branch', '', None)})
