...
__istioctl_handle_activeHelp() {
    ...
    if ((${#activeHelp[*]} != 0)); then
        if [ -z $COMP_TYPE ]; then
            ...
            printf "%s\n" "${activeHelp[@]}"
            printf "\n"
            __istioctl_reprint_commandLine
            return
        fi
        ...
        if [ $COMP_TYPE -eq 63 ]; then
            ...
            if ((${#COMPREPLY[*]} == 0)); then
                ...
                if (((directive & shellCompDirectiveNoFileComp) == 0)); then
                    __istioctl_debug "Listing files"
                    _filedir
                fi
            fi
...
__istioctl_handle_standard_completion_case() {
    ...
    if ((${#COMPREPLY[*]} == 1)); then
        ...
        __istioctl_debug "Removed description from single completion, which is now: ${COMPREPLY[0]}"
    else
