...
func main() {
	...
	for _, branch := range jobs.Branches {
		...
		case "check":
			checkConfig(output, jobs.Repo, branch)
		case "print":
			fallthrough
		default:
			printConfig(output)
...
func checkConfig(jobs config.JobConfig, fqRepo string, branch string)  {
	r := strings.Split(fqRepo, "/")
	org, repo := r[0], r[1]
	key := fmt.Sprintf("%s.%s.%s.yaml", org, repo, branch)

	currentConfigFile := path.Join("..", "cluster", "jobs", org, repo, key)
	current, err := ioutil.ReadFile(currentConfigFile)
	if err != nil {
		exit(err, "failed to read current config for " + currentConfigFile)
	}
	...
	output := []byte("# THIS FILE IS AUTOGENERATED. See prow/config/README.md\n")
	output = append(output, newConfig...)

	if !bytes.Equal(current, output) {
		exit(fmt.Errorf("generated config is different than file %v", currentConfigFile), "")
	}
}
...
func writeConfig(jobs config.JobConfig, fqRepo string, branch string) {
	...
	outdir := path.Join("..", "cluster", "jobs", org, repo)
