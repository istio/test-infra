#!/usr/bin/env python
#This script checks if every package satisfies package code coverage requirments.
...
class PkgChecker:
    ...
    def check(self):
        ...
        if not failed_pkgs:
            print ("All packages passed code coverage requirements!")
            return 0
        else:
            print ("Following package(s) failed to meet requirements:\n \
            Package Name    Actual Coverage    Requirement")
            for pkg in failed_pkgs:
                print pkg,
            print ("Failed to satisfy package coverage")
            return 2
