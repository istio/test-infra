...
get_opts() {
  if opt="$(getopt -o i -l branch:,sha:,org:,repo:,title:,body:,user:,email:,modifier:,script-path:,cmd:,token-path:,token: -n "$(basename "$0")" -- "$@")"; then
    eval set -- "$opt"
  else
...
evaluate_opts() {
  export AUTOMATOR_ORG="$org" AUTOMATOR_REPO="$repo" AUTOMATOR_BRANCH="$branch" AUTOMATOR_SHA="$sha" AUTOMATOR_SHA_SHORT="$sha_short"
  title="$(eval echo "${title_tmpl:-Automator: update $org/$repo@$branch-$modifier}")"
  body="$(eval echo "${body_tmpl:-Generated by Automator - $(date -uIseconds)}")"
}
